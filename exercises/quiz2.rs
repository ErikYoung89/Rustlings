    ...
    pub fn transformer(input: Vec<(String,Command)>) -> Vec<String> {
        ...
        for (string, command) in input.iter() {
            ...
            // Solution from Chris Biscardi
                ...
                Command::Append(num) => {
                    output.push(format!(
                        "{string}{}","bar".repeat(*num)
                    ));
                }
            }
            //Solution I found on Reddit
            //  Command::Uppercase => {output.push(string.to_uppercase())}
            //  Command::Trim => {output.push(string.trim().to_string())}
            //  Command::Append(n) =>  {output.push(append_bar(&string, *n))}
                // see append_bar above

            // my original solution
            // let mut string: &mut String = &mut String::from(string);
            // match command {
            //     Command::Uppercase => {output.push(string.to_uppercase())}
            //     Command::Trim => {output.push(string.trim().to_string())}
            //     Command::Append(i) =>  {string.push_str("bar".repeat(*i).as_str());
